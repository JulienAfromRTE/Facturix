...
def parse_rdi(rdi_path):
    """
    Parse le fichier RDI et retourne (data, articles, data_multi, bg23_blocks).
    - data : dict des champs d'en-tête (hors articles et hors blocs BG-23 répétitifs)
    - articles : liste de dicts, un par bloc article (BG25/BG26/BG29/BG30/BG31)
    - data_multi : {tag_upper: [(record_type, value), ...]} toutes occurrences
    - bg23_blocks : liste de dicts, un par bloc BG-23 (quand le RDI les répète)
    """
    data = {}
    articles = []
    bg23_blocks = []
    ...
    # Préfixe des tags BG-23 (détail TVA au niveau en-tête)
    BG23_PREFIX = 'GS_FECT_EINV-BG23-'
    ...
    for record_type, tag, value in parsed_lines:
        ...
        # Gestion des blocs BG-23 répétitifs (détail TVA en-tête)
            ...
            if current_bg23 is not None:
                current_bg23[tag] = value
            elif tag not in data:
                # Ancien format : BT118_0 seul dans le RDI, aucun bloc ouvert → header
                data[tag] = value
...
def reembed_xml_in_pdf(pdf_path, xml_bytes):
    """Remplace le XML Factur-X embarqué dans le PDF par xml_bytes. Retourne BytesIO du PDF modifié."""
    import zlib
    with pikepdf.open(pdf_path) as pdf:
        try:
            ef_names = pdf.Root['/Names']['/EmbeddedFiles']['/Names']
        except (KeyError, AttributeError, TypeError):
            raise ValueError('Aucun fichier XML embarqué trouvé dans ce PDF')
        replaced = False
        for i in range(0, len(ef_names), 2):
            if str(ef_names[i]).lower().endswith('.xml'):
                file_spec = ef_names[i + 1]
                ef_stream = file_spec['/EF']['/F']
                original_filter = ef_stream.get('/Filter')
                if original_filter == pikepdf.Name('/FlateDecode'):
                    # Compresser manuellement : pikepdf.write(data, filter=FlateDecode)
                    # attend des données déjà compressées, pas des données brutes.
                    compressed = zlib.compress(xml_bytes)
                    ef_stream.write(compressed, filter=pikepdf.Name('/FlateDecode'))
                else:
                    ef_stream.write(xml_bytes)
                try:
                    if '/Params' in file_spec:
                        params = file_spec['/Params']
                        if '/Size' in params:
                            params['/Size'] = pikepdf.Integer(len(xml_bytes))
                except Exception:
                    pass
                replaced = True
                break
        if not replaced:
            raise ValueError('Aucun fichier XML (.xml) trouvé dans EmbeddedFiles')
        out = io.BytesIO()
        pdf.save(out)
        out.seek(0)
        return out
